...
class MassSpecDataset(Dataset):
    ...
    def load_data(self):
        ...
        if isinstance(self.pth, str):
            print(f"path specified and loaded: {self.pth}")
...
class MassDataset(Dataset):
    """
    Dataset for mass-binned retrieval data.
    Expects a JSON dict: {mass_bin: [smiles, ...], ...}

    Behaves similarly to UnlabeledDataset but for dicts instead of lists.
    """

    def __init__(
        self,
        raw_pth: T.Union[str, Path],
        key_type: T.Optional[type] = None,
        value_type: T.Optional[type] = None,
        item_fn: T.Optional[T.Callable[[dict, T.Any], dict]] = None,
        collate_fn: T.Optional[T.Callable[[T.Iterable[dict]], dict]] = None,
    ):
        super().__init__()
        ...
        if not isinstance(raw, dict):
            raise ValueError("MassDataset expects a JSON dict {mass_bin: values}.")

        # Optional type validation
        if key_type is not None:
            if not all(isinstance(k, key_type) for k in raw.keys()):
                raise ValueError(f"Expected all keys to be of type {key_type}.")

        if value_type is not None:
            if not all(isinstance(v, value_type) for v in raw.values()):
                raise ValueError(f"Expected all values to be of type {value_type}.")

        self.raw = raw
        self.keys = list(raw.keys())
        self.data = raw  # dict

        # Default item_fn returns {"mass": key, "items": value}
        self._item_fn = item_fn or (lambda data, key: {"mass": key, "items": data[key]})

        # Default collate_fn uses torch default_collate on list of dicts
        self._collate_fn = collate_fn or (lambda b: default_collate(list(b)))

    def json_load(self, pth: Path) -> dict:
        with open(pth, "r") as fh:
            return json.load(fh)
    ...
    def __getitem__(self, idx: int) -> dict:
        key = self.keys[idx]
        return self._item_fn(self.data, key)
