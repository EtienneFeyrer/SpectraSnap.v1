...
class MolToGraph(MolTransform):
    ...
    def check_valid_smiles(self, smile:str )-> bool:
        ...
        if "." in canon:
            #print(". failure in compound  {}".format(smiles))
            return False
            
        if atoms is None:
            print("none atom failure in compound  {}".format(smile))
            return False

        if len(atoms) == 1:
            #print("found unit length compound {}".format(compound.decode()))
            return False
        atom = [a in self.element_list for a in atoms]            
        if not all(atom):
            return False
        g = chemutils.mol_to_bigraph(mol, node_featurizer=self.node_featurizer, edge_featurizer=self.edge_featurizer, add_self_loop = True,
                                num_virtual_nodes = 0)
        if g is None:
            ...
            return False
        # if no exception was raised, pass molecule
        return True