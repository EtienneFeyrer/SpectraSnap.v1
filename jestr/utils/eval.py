...
def get_target(candidates, labels):
    label_mask = pd.Series(labels, dtype=bool)
    if not label_mask.any():
        return None
    return pd.Series(candidates)[label_mask].iloc[0]
...
def borda_count(candidates, score_lists, target):
    ...
    Input: candidates(f.E.: list of smiles), 
           score_lists(f.E.: list of lists of distances), 
           target(f.E.: target smiles)
    Output: rank of the target(f.E.: target smiles)'''
